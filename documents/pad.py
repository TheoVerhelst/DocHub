...
class Pad:
    ...
    def _row_col_are_valid(self, row, col):
        ...
        """
        return 0 <= row and row < len(self.lines) and 0 <= col and \
          (col < len(self.lines[row]) or (row==len(self.lines)-1 and col==len(self.lines[row])))
    ...
    def _row_col_to_char(self, row, col):
        ...
        """
        if not self._row_col_are_valid(row, col):
            raise ValueError ("row %s col %s not valid"%(row, col)) #(row, col) is not a valid position
